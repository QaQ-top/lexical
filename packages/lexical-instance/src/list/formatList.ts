 ...
 */
export function $insertInstanceList(listType: ListType): void {
  ...
  if (selection !== null) {
    ...
    if ($isRangeSelection(selection)) {
      ...
      if ($isRootOrShadowRoot(anchorNode)) {
        ...
        const list = $createInstanceListNode(listType);
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if ($findMatchingParent(node, (node) => $isInstanceTitleNode(node))) {
        // 标题不让替换标题格式
        continue;
      }
      ...
      if (
        ...
        $createInstanceListOrMerge(node, listType);
        continue;
      }
      ...
      if (
        parent &&
        $findMatchingParent(node, (node) => $isInstanceNode(node)) &&
        !$findMatchingParent(node, (node) => $isListNode(node))
      ) {
        $createInstanceListOrMerge(parent, listType);
        continue;
      }
      ...
      while (parent != null) {
        ...
        if ($isListNode(parent)) {
          if (!handled.has(parentKey)) {
            const newListNode = $createInstanceListNode(listType);
...
function $createInstanceListOrMerge(
  node: ElementNode,
  listType: ListType,
): ListNode {
  if ($isListNode(node)) {
...
export function $handleIndent(listItemNode: ListItemNode): void {
  ...
  if (isNestedListNode(nextSibling) && isNestedListNode(previousSibling)) {
    ...
    if ($isListNode(parent)) {
      const newListItem = $createInstanceListItemNode()
        .setTextFormat(parent.getTextFormat())
        .setTextStyle(parent.getTextStyle());
      const newList = $createInstanceListNode(parent.getListType())
        .setTextFormat(parent.getTextFormat())
        .setTextStyle(parent.getTextStyle());
...
export function $handleOutdent(listItemNode: ListItemNode): void {
  ...
  if (
    ...
    if (listItemNode.is(firstChild)) {
      ...
      const previousSiblingsListItem = $createInstanceListItemNode();
      const previousSiblingsList = $createInstanceListNode(listType);
      previousSiblingsListItem.append(previousSiblingsList);
      listItemNode
        .getPreviousSiblings()
        .forEach((sibling) => previousSiblingsList.append(sibling));
      const nextSiblingsListItem = $createInstanceListItemNode();
      const nextSiblingsList = $createInstanceListNode(listType);
...
export function $handleListInsertParagraph(): boolean {
  ...
  if ($isRootOrShadowRoot(grandparent)) {
    replacementNode = $createInstanceParagraphNode();
    topListNode.insertAfter(replacementNode);
  } else if ($isListItemNode(grandparent)) {
    replacementNode = $createInstanceListItemNode();
