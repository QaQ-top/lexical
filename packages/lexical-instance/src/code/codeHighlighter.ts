...
import {$isInstanceCodeNode, InstanceCodeNode} from './index';
...
export function $getStartOfCodeInLine(
  anchor: InstanceCodeHighlightNode | TabNode,
  offset: number,
): null | {
  node: InstanceCodeHighlightNode | TabNode | LineBreakNode;
  offset: number;
} {
  let last: null | {
    node: InstanceCodeHighlightNode | TabNode | LineBreakNode;
  ...
  let lastNonBlank: null | {node: InstanceCodeHighlightNode; offset: number} =
    null;
  let node: null | InstanceCodeHighlightNode | TabNode | LineBreakNode = anchor;
  ...
  while (true) {
    if (nodeOffset === 0) {
      ...
      invariant(
        $isInstanceCodeHighlightNode(node) ||
          $isTabNode(node) ||
          $isLineBreakNode(node),
        'Expected a valid Code Node: InstanceCodeHighlightNode, TabNode, LineBreakNode',
      );
    ...
    if ($isInstanceCodeHighlightNode(node) && character !== ' ') {
      lastNonBlank = {
        node,
        offset: nodeOffset,
      };
  ...
  if (offset < anchor.getTextContentSize()) {
    if ($isInstanceCodeHighlightNode(anchor)) {
      codeCharacterAtAnchorOffset = anchor.getTextContent()[offset];
    }
  } else {
    const nextSibling = anchor.getNextSibling();
    if ($isInstanceCodeHighlightNode(nextSibling)) {
...
function findNextNonBlankInLine(
  anchor: LexicalNode,
  offset: number,
): null | {node: InstanceCodeHighlightNode; offset: number} {
  ...
  while (true) {
    if (
      !$isInstanceCodeHighlightNode(node) ||
      nodeOffset === nodeTextContentSize
    ) {
      node = node.getNextSibling();
      if (node === null || $isLineBreakNode(node)) {
        return null;
      }
      if ($isInstanceCodeHighlightNode(node)) {
        nodeOffset = 0;
        nodeTextContent = node.getTextContent();
...
export function $getEndOfCodeInLine(
  anchor: InstanceCodeHighlightNode | TabNode,
): InstanceCodeHighlightNode | TabNode {
...
function $textNodeTransform(
  ...
  // Since InstanceCodeNode has flat children structure we only need to check
...
function updateCodeGutter(node: InstanceCodeNode, editor: LexicalEditor): void {
...
//
// Using extra cache (`nodesCurrentlyHighlighting`) since both InstanceCodeNode and InstanceCodeHighlightNode
// transforms might be called at the same time (e.g. new CodeHighlight node inserted) and
// in both cases we'll rerun whole reformatting over InstanceCodeNode, which is redundant.
    ...
    if (typeof token === 'string') {
      ...
      for (let i = 0; i < partialsLength; i++) {
        ...
        if (part === '\n' || part === '\r\n') {
          ...
          nodes.push($createInstanceCodeHighlightNode(part, type));
        }
      }
    } else {
      const {content} = token;
...
function $updateAndRetainSelection(
  ...
  if (!$isInstanceCodeNode(node) || !node.isAttached()) {
    return;
  }
...
function isEqual(nodeA: LexicalNode, nodeB: LexicalNode): boolean {
  ...
  return (
    ($isInstanceCodeHighlightNode(nodeA) &&
      $isInstanceCodeHighlightNode(nodeB) &&
...
function $isSelectionInCode(selection: null | BaseSelection): boolean {
  ...
  if (anchorNode.is(focusNode) && $isInstanceCodeNode(anchorNode)) {
    return true;
  }
  const anchorParent = anchorNode.getParent();
  return (
    $isInstanceCodeNode(anchorParent) && anchorParent.is(focusNode.getParent())
  );
}

function $getCodeLines(
  selection: RangeSelection,
): Array<Array<InstanceCodeHighlightNode | TabNode>> {
  const nodes = selection.getNodes();
  const lines: Array<Array<InstanceCodeHighlightNode | TabNode>> = [[]];
  if (nodes.length === 1 && $isInstanceCodeNode(nodes[0])) {
    return lines;
  }
  let lastLine: Array<InstanceCodeHighlightNode | TabNode> = lines[0];
  for (let i = 0; i < nodes.length; i++) {
    ...
    invariant(
      $isInstanceCodeHighlightNode(node) ||
        $isTabNode(node) ||
        $isLineBreakNode(node),
      'Expected selection to be inside CodeBlock and consisting of InstanceCodeHighlightNode, TabNode and LineBreakNode',
    );
    if ($isLineBreakNode(node)) {
...
function $handleTab(shiftKey: boolean): null | LexicalCommand<void> {
  ...
  invariant(
    $isInstanceCodeNode(firstNode) ||
      $isInstanceCodeHighlightNode(firstNode) ||
      $isTabNode(firstNode) ||
      $isLineBreakNode(firstNode),
    'Expected selection firstNode to be InstanceCodeHighlightNode or TabNode',
  );
  if ($isInstanceCodeNode(firstNode)) {
    return indentOrOutdent;
  }
...
function $handleMultilineIndent(type: LexicalCommand<void>): boolean {
  ...
  invariant(
    $isInstanceCodeNode(firstNode) ||
      ...
      $isTabNode(firstNode) ||
      $isLineBreakNode(firstNode),
    'Expected selection firstNode to be InstanceCodeHighlightNode or CodeTabNode',
  );
  if ($isInstanceCodeNode(firstNode)) {
    // InstanceCodeNode is empty
    if (type === INDENT_CONTENT_COMMAND) {
...
function $handleShiftLines(
  ...
  if (
    ...
    !($isInstanceCodeHighlightNode(anchorNode) || $isTabNode(anchorNode)) ||
    !($isInstanceCodeHighlightNode(focusNode) || $isTabNode(focusNode))
  ) {
    return false;
  }
  if (!event.altKey) {
  ...
  const maybeInsertionPoint =
    $isInstanceCodeHighlightNode(sibling) ||
    $isTabNode(sibling) ||
...
export function registerCodeHighlighting(
  ...
  if (!editor.hasNodes([InstanceCodeNode, InstanceCodeHighlightNode])) {
    throw new Error(
      'CodeHighlightPlugin: InstanceCodeNode or InstanceCodeHighlightNode not registered on editor',
    );
  }
  ...
  return mergeRegister(
        ...
        editor.update(() => {
          for (const [key, type] of mutations) {
            if (type !== 'destroyed') {
              ...
              if (node !== null) {
                updateCodeGutter(node as InstanceCodeNode, editor);
              }
            }
          }
        });
    ...
    editor.registerNodeTransform(InstanceCodeHighlightNode, (node) =>
      $textNodeTransform(node, editor, tokenizer as Tokenizer),
    ),
    ...
    editor.registerCommand(
      ...
      (event) => {
        ...
        if (
          ...
          anchor.offset === 0 &&
          anchorNode.getPreviousSibling() === null &&
          $isInstanceCodeNode(anchorNode.getParentOrThrow())
        ) {
          event.preventDefault();
          return true;
        }
