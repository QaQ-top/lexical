...
// [new]
/** 实例类型的节点是否需要转换*/
export function $instanceNodeChangeOfStyle<T extends ElementNode>(
  prevNode: T,
  element: T,
): {
  /** element 传递进来的节点可能被替换 */
  element: T;
  /** 是否需要跟换格式 */
  value: boolean;
  /** 删除某些节点 */
  ...
  if ($isInstanceNode(parent)) {
    // 如果父级是 instance 类型，说明 prevNode 是一个实例段落
    // 改变的格式不是原本的类型时，才发生替换
    if (prevNode.getType() !== element.getType()) {
      const children = prevNode.getChildren();
      element.append(...children);
      prevNode.append(element);
      // console.log('添加');
    } else {
      data.value = false;
      // console.log('替换相同，不处理');
      return data;
    }
  } else {
    // 如果父级类型与需要替换的类型相同，那么直接将当前 node 内的子，全部放到当前node的父级下
    if (parent && parent.getType() === element.getType()) {
      const children = prevNode.getChildren();
      // 只有新的节点才能触发 selectionchange
      const copy = $copyNode(parent);
      parent.replace(copy);
      copy.append(...children);
      data.removeNode = () => {
        prevNode.remove();
      };
      data.element = copy as T;
      // console.log('还原');
    } else {
...
export function $setInstanceBlocksType<T extends ElementNode>(
  selection: BaseSelection | null,
  $createElement: () => T,
  $afterCreateElement: (
    prevNodeSrc: ElementNode,
  ...
  const pendingRun: (() => void)[] = [];
  for (const [key, prevNode] of blockMap) {
    let element = $createElement();
    $afterCreateElement(prevNode, element);
    // [源码修改] [bug]
    const changeState = $instanceNodeChangeOfStyle(prevNode as T, element);
    if (!changeState.value) {
      continue;
    } else {
      element = changeState.element;
      pendingRun.push(changeState.removeNode);
    }
    if (newSelection) {
      if (key === newSelection.anchor.key) {
        newSelection.anchor.set(
  ...
  // 必须保证选中处理完后才能安全的删除之前有选中的节点
  pendingRun.forEach((run) => run());
  if (newSelection && selection.is($getSelection())) {
    $setSelection(newSelection);
  }
}
